...
public:
	...
	int test_group_object_rw(bool test) {

		// Test reading and writing into tags group and projects group.
		if (!test) return 0;

		TIMEY_WARN("***********  GROUP OBJ DB READ WRITE TEST ***********\n\n");
...
int main(int argc, char** argv) {
	...
	TestDBFunctions test_instance= test_db_path;

	test_instance.test_basic_rw(false);
	int res = test_instance.test_object_group_update(true);
